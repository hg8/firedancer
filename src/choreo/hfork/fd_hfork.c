...
/* fd_hfork maintains three pools and three maps:
   ...
   blk_pool (capacity max): pool of blk_t elements.  Each blk_t stores
            per-block metadata (our_bank_hash, replayed, dead, forked,
            checked) and owns a bhm_dlist of all bhm entries sharing the
            same block_id.
...
/* check returns -1 if the block is dead or 52% of stake agreed on a
   bank hash different from ours (hard fork), 1 if 52% of stake agreed
   on the same bank hash as ours (confirming), or 0 otherwise (not
   enough info yet or already checked). */
...
check( blk_t * blk,
  ...
  if( FD_LIKELY( blk->checked ) ) return 0;
  if( FD_LIKELY( !blk->replayed && !blk->dead ) ) return 0;
  double pct = (double)bhm->stake * 100.0 / (double)total_stake;
  if( FD_LIKELY( pct < 52.0 ) ) return 0;
...
fd_hfork_count_vote( fd_hfork_t *        hfork,
  ...
  if( FD_UNLIKELY( vte_map_ele_query_const( hfork->vte_map, &vte_key, NULL, hfork->vte_pool ) ) ) return 0;

  /* Only process newer votes (by vote slot) from a given voter. */

  if( FD_UNLIKELY( vtr->vte_cnt && vte_dlist_ele_peek_tail_const( vtr->vte_dlist, hfork->vte_pool )->slot >= slot ) ) return 0;
  ...
  if( FD_UNLIKELY( !blk ) ) {
    blk                = blk_pool_ele_acquire( hfork->blk_pool );
    blk->block_id      = *block_id;
    blk->our_bank_hash = (fd_hash_t){0};
    blk->checked       = 0;
    blk->dead          = 0;
    blk->forked        = 0;
    blk->replayed      = 0;
  ...
  if( FD_LIKELY( opt_bank_hash_cnt ) ) *opt_bank_hash_cnt = blk->bhm_cnt;

  /* Check for hard forks. */

  return check( blk, bhm, total_stake );
}
...
fd_hfork_record_our_bank_hash( fd_hfork_t * hfork,
  ...
  if( FD_UNLIKELY( !blk ) ) {
    blk                = blk_pool_ele_acquire( hfork->blk_pool );
    blk->block_id      = *block_id;
    blk->our_bank_hash = bank_hash ? *bank_hash : (fd_hash_t){0};
    blk->checked       = 0;
    blk->dead          = !bank_hash;
    blk->forked        = 0;
    blk->replayed      = !!bank_hash;
    blk->bhm_cnt       = 0;
    blk_map_ele_insert( hfork->blk_map, blk, hfork->blk_pool );
    return 0;
  }

  /* Set metadata on the blk. */

  if( FD_LIKELY( bank_hash ) ) { blk->dead = 0; blk->replayed = 1; blk->our_bank_hash = *bank_hash; }
  else                         { blk->dead = 1; blk->replayed = 0;                                   }
  blk->checked = 0; /* re-check all bhm entries */
  ...
  for( bhm_dlist_iter_t iter = bhm_dlist_iter_fwd_init( blk->bhm_dlist, hfork->bhm_pool );
    ...
    int r = check( blk, bhm, total_stake );
    if( FD_UNLIKELY( r ) ) result = r;
  }
  return result;
}
