...
confirm_block( fd_tower_tile_t * ctx,
  ...
  for( ulong i = 0UL; i < FD_TOWER_SLOT_CONFIRMED_LEVEL_CNT; i++ ) {
    ...
    while( FD_LIKELY( ghost_anc && notar_anc && notar_anc->level < levels[i] ) ) {
      notar_anc = fd_notar_blk_query( ctx->notar, &ghost_anc->id );
...
count_vote( fd_tower_tile_t * ctx,
  ...
  ulong bank_hash_cnt = 0UL;
  int hfork_err = fd_hfork_count_vote( ctx->hfork, vote_acc, their_block_id, their_bank_hash, their_last_vote->slot, their_stake, total_stake, &bank_hash_cnt );
  if( FD_UNLIKELY( bank_hash_cnt>1 ) ) ctx->metrics.hfork_forked_slot = fd_ulong_max( ctx->metrics.hfork_forked_slot, their_last_vote->slot );
  ...
  if( FD_UNLIKELY( !tower_blk ) ) { ctx->metrics.vote_txn_unknown_slot++; return; }; /* we haven't replayed this block yet */
...
replay_slot_completed( fd_tower_tile_t *            ctx,
  ...
  update_hfork_metrics( ctx, hfork_err, slot_completed->slot, &slot_completed->block_id );
  ...
  if( FD_UNLIKELY( out.root_slot!=ULONG_MAX ) ) {
    ...
    if( FD_UNLIKELY( fd_notar_root( ctx->notar )==ULONG_MAX || oldr_tower_blk->epoch+1==newr_tower_blk->epoch ) ) {
      FD_TEST( newr_tower_blk->epoch==slot_completed->epoch ); /* new root's epoch must be same as current slot_completed */
      fd_notar_update_voters( ctx->notar, ctx->tower_voters, ctx->tower_stakes, out.root_slot );
...
scratch_footprint( fd_topo_tile_t const * tile ) {
  ...
  /* auth_vtr_keyswitch */
  l = FD_LAYOUT_APPEND( l, fd_eqvoc_align(),         fd_eqvoc_footprint( slot_max, fec_max, slot_max, FD_VOTER_MAX ) );
  ...
  l = FD_LAYOUT_APPEND( l, fd_notar_align(),         fd_notar_footprint( slot_max, FD_VOTER_MAX )                        );
...
metrics_write( fd_tower_tile_t * ctx ) {
  ...
  FD_MGAUGE_SET( TOWER, HFORK_FORKED_SLOT,    ctx->metrics.hfork_forked_slot    );
}
...
returnable_frag( fd_tower_tile_t *   ctx,
  ...
  case IN_KIND_REPLAY: {
    ...
    if( FD_LIKELY( sig==REPLAY_SIG_TXN_EXECUTED ) ) {
      ...
      int hfork_err = fd_hfork_record_our_bank_hash( ctx->hfork, &slot_dead->block_id, NULL, fd_ghost_root( ctx->ghost )->total_stake );
      update_hfork_metrics( ctx, hfork_err, slot_dead->slot, &slot_dead->block_id );
    }
    return 0;
  }
  case IN_KIND_SHRED: {
...
unprivileged_init( fd_topo_t *      topo,
  ...
  fd_tower_tile_t * ctx = FD_SCRATCH_ALLOC_APPEND( l, alignof(fd_tower_tile_t), sizeof(fd_tower_tile_t)                               );
  void  * auth_vtr      = FD_SCRATCH_ALLOC_APPEND( l, auth_vtr_align(),         auth_vtr_footprint()                                  );
  void  * eqvoc         = FD_SCRATCH_ALLOC_APPEND( l, fd_eqvoc_align(),         fd_eqvoc_footprint( slot_max, fec_max, slot_max, FD_VOTER_MAX ) );
  ...
  void  * notar         = FD_SCRATCH_ALLOC_APPEND( l, fd_notar_align(),         fd_notar_footprint( slot_max, FD_VOTER_MAX )                        );
  ...
  ctx->eqvoc              = fd_eqvoc_join       ( fd_eqvoc_new       ( eqvoc, slot_max, fec_max, slot_max, FD_VOTER_MAX, ctx->seed )           );
  ctx->ghost              = fd_ghost_join       ( fd_ghost_new       ( ghost, blk_max, FD_VOTER_MAX, ctx->seed )                               );
  ctx->hfork              = fd_hfork_join       ( fd_hfork_new       ( hfork, slot_max, FD_VOTER_MAX, ctx->seed ) );
  ctx->hard_fork_fatal    = tile->tower.hard_fork_fatal;
  ctx->notar              = fd_notar_join       ( fd_notar_new       ( notar, slot_max, FD_VOTER_MAX, ctx->seed )                               );
