...
/* Returns 1 if the (full_hash, incr_hash) combination signals that a
   peer's incremental data should be cleared.  Convention: full_hash
   non-NULL means the caller is providing full snapshot info; incr_hash
   NULL in that context means the peer no longer offers an incremental
   snapshot, so stale incremental data must be wiped.  When both are
   NULL the caller is not updating snapshot info at all, so existing
   incremental data is preserved. */
static inline int
fd_sspeer_is_clear_incr( uchar const full_hash[ FD_HASH_FOOTPRINT ],
                         uchar const incr_hash[ FD_HASH_FOOTPRINT ] ) {
  return full_hash && !incr_hash;
}

/* Validates slot arguments for both new and existing peers.  Returns 0
   on success, -1 on failure.  The caller passes in the effective
   (already-resolved) full_slot and incr_slot values.

   Two invariants are enforced:
   1. Clearing incremental (full_hash present, incr_hash absent) requires
      incr_slot==FD_SSPEER_SLOT_UNKNOWN.
   2. When both slots are known, incr_slot must be >= full_slot. */
static int
fd_sspeer_validate_slot_args( ulong       full_slot,
                              ulong       incr_slot,
                              uchar const full_hash[ FD_HASH_FOOTPRINT ],
                              uchar const incr_hash[ FD_HASH_FOOTPRINT ] ) {
  if( FD_UNLIKELY( fd_sspeer_is_clear_incr( full_hash, incr_hash ) && incr_slot!=FD_SSPEER_SLOT_UNKNOWN ) ) {
    FD_LOG_WARNING(( "clear incremental requires input incremental slot to be FD_SSPEER_SLOT_UNKNOWN" ));
    return -1;
  }
  ...
  if( FD_UNLIKELY( incr_slot!=FD_SSPEER_SLOT_UNKNOWN &&
                   ...
                   incr_slot<full_slot ) ) {
    FD_LOG_WARNING(( "incremental slot %lu must be >= full slot %lu", incr_slot, full_slot ));
    return -1;
  }
...
/* Updates a peer's score with new values for latency and/or resolved
   full/incremental slots.  On failure returns -1 without modifying
   the peer or any data structure, otherwise 0. */
static int
fd_sspeer_selector_update( fd_sspeer_selector_t * selector,
  ...
  int clear_incr = fd_sspeer_is_clear_incr( full_hash, incr_hash );
  ...
  if( FD_UNLIKELY( fd_sspeer_validate_slot_args( peer_full_slot, peer_incr_slot, full_hash, incr_hash ) ) ) return -1;
...
fd_sspeer_selector_update_on_ping( fd_sspeer_selector_t * selector,
  ...
  for(;;) {
    ...
    /* update cannot fail: slots are FD_SSPEER_SLOT_UNKNOWN, hashes
       are NULL, so no validation constraint can be violated. */
    (void)fd_sspeer_selector_update( selector, peer, latency, FD_SSPEER_SLOT_UNKNOWN, FD_SSPEER_SLOT_UNKNOWN, NULL, NULL );
...
fd_sspeer_selector_add( fd_sspeer_selector_t * selector,
  ...
  if( FD_LIKELY( peer ) ) {
    ...
    if( FD_UNLIKELY( fd_sspeer_validate_slot_args( full_slot, incr_slot, full_hash, incr_hash ) ) ) {
      return FD_SSPEER_SCORE_INVALID;
    }
  ...
  if( FD_LIKELY( selector->incremental_snapshot_fetch ) ) {
    ...
    if( FD_UNLIKELY( incr_slot!=FD_SSPEER_SLOT_UNKNOWN ) ) {
      if( FD_UNLIKELY( selector->cluster_slot.incremental!=FD_SSPEER_SLOT_UNKNOWN &&
                       incr_slot<=selector->cluster_slot.incremental ) ) return;
      else if( FD_UNLIKELY( selector->cluster_slot.incremental==FD_SSPEER_SLOT_UNKNOWN &&
                            incr_slot<selector->cluster_slot.full ) ) return;
