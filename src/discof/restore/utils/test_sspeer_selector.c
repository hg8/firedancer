...
test_update_on_resolve( fd_sspeer_selector_t * selector,
  ...
  FD_TEST( best.incr_slot==FD_SSPEER_SLOT_UNKNOWN );
  FD_TEST( best.score==FD_SSPEER_SCORE_INVALID );
  FD_TEST(fd_sspeer_selector_update_on_resolve( selector, key_A, FD_SSPEER_SLOT_UNKNOWN, cluster_incr_slot, NULL, NULL )==FD_SSPEER_UPDATE_SUCCESS  );
  best = fd_sspeer_selector_best( selector, 0, FD_SSPEER_SLOT_UNKNOWN );
  FD_TEST( !best.addr.l );
  FD_TEST( best.full_slot==FD_SSPEER_SLOT_UNKNOWN );
  FD_TEST( best.incr_slot==FD_SSPEER_SLOT_UNKNOWN );
...
test_add_clears_incremental( fd_sspeer_selector_t * selector,
  ...
  /* New gossip message for peer A arrives without an incremental.
     fd_sspeer_selector_add with non-NULL full_hash but NULL incr_hash
     should clear the peer's stale incremental data. */
  ...
  /* Peer A should still be valid for full selection with incr_slot
     now set to FD_SSPEER_SLOT_UNKNOWN and incr_hash zeroed. */
  best = fd_sspeer_selector_best( selector, 0, FD_SSPEER_SLOT_UNKNOWN );
  FD_TEST( best.addr.l==addr_A.l );
  FD_TEST( best.full_slot==cluster_full_slot );
  FD_TEST( best.incr_slot==FD_SSPEER_SLOT_UNKNOWN );
  ...
  /* Clear incremental while also advancing full_slot, for a peer with
     a new gossip message with a higher full_slot but no incremental. */
  ulong new_full_slot = cluster_full_slot + 1UL;
  FD_TEST( fd_sspeer_selector_add( selector, key_A, addr_A, FD_SSPEER_LATENCY_UNKNOWN,
                                   new_full_slot, FD_SSPEER_SLOT_UNKNOWN,
                                   temp_full_hash, NULL )==2UL*1000UL*1000UL );

  /* Peer A should not be an incremental candidate for old base slot
     (full_slot mismatch) or new base slot
  ...
  /* Restore full_slot back for subsequent tests. */
  FD_TEST( fd_sspeer_selector_add( selector, key_A, addr_A, FD_SSPEER_LATENCY_UNKNOWN,
                                   cluster_full_slot, FD_SSPEER_SLOT_UNKNOWN,
                                   temp_full_hash, NULL )==2UL*1000UL*1000UL );
  ...
  /* Peer A should still be returned for full selection. */
  best = fd_sspeer_selector_best( selector, 0, FD_SSPEER_SLOT_UNKNOWN );
  FD_TEST( best.addr.l==addr_A.l );
  FD_TEST( best.full_slot==cluster_full_slot );
  FD_TEST( best.incr_slot==FD_SSPEER_SLOT_UNKNOWN );
  ...
  /* Peer A should NOT be returned for incremental selection. */
  best = fd_sspeer_selector_best( selector, 1, cluster_full_slot );
  FD_TEST( !best.addr.l );
  FD_TEST( best.score==FD_SSPEER_SCORE_INVALID );
  ...
  /* add() with full_hash==NULL, incr_hash!=NULL: the full_hash should
     ...
     clear_incr is false because full_hash is NULL. */
...
test_on_resolve_clears_incremental( fd_sspeer_selector_t * selector,
  ...
  /* Call update_on_resolve with full_hash non-NULL and incr_hash NULL.
     This triggers the clear_incr path through the resolve codepath. */
...
test_score_saturation( fd_sspeer_selector_t * selector,
  ...
  /* Cluster incremental is FD_SSPEER_SLOT_UNKNOWN (initial state):
     ...
     slots_behind=0.  score = 5_000_000. */
  ...
  /* Unresolved full slot with valid incr_slot:
     full_slot==FD_SSPEER_SLOT_UNKNOWN causes the incr_slot to be
     ignored, falling back to DEFAULT_SLOTS_BEHIND.
     score = 5_000_000 + 1_000_000*1000 = 1_005_000_000. */
  FD_TEST( add_peer( selector, key, addr, FD_SSPEER_SLOT_UNKNOWN, 10500UL, 5UL*1000UL*1000UL )==5UL*1000UL*1000UL + 1000UL*1000UL*1000UL );
  fd_sspeer_selector_remove( selector, key );
...
test_cluster_slot_monotonicity( fd_sspeer_selector_t * selector_incr,
  ...
  /* incr_slot==stored incr (20200), both valid => rejected. */
  fd_sspeer_selector_process_cluster_slot( selector_incr, 20000UL, 20200UL );
  cs = fd_sspeer_selector_cluster_slot( selector_incr );
  FD_TEST( cs.full==20000UL );
  FD_TEST( cs.incremental==20200UL );
  ...
  fd_sspeer_selector_process_cluster_slot( selector_incr, 20000UL, 20199UL );
  cs = fd_sspeer_selector_cluster_slot( selector_incr );
  FD_TEST( cs.full==20000UL );
  FD_TEST( cs.incremental==20200UL );

  /* incr_slot > stored incr => accepted. */
  fd_sspeer_selector_process_cluster_slot( selector_incr, 20000UL, 20201UL );
  cs = fd_sspeer_selector_cluster_slot( selector_incr );
  FD_TEST( cs.full==20000UL );
...
test_ping_preserves_cleared_incremental( fd_sspeer_selector_t * selector,
  ...
  /* Verify that update_on_ping does NOT restore stale incremental data
     after a peer's incremental was explicitly cleared.
     ...
     FD_SSPEER_SLOT_UNKNOWN for both slots.  In fd_sspeer_selector_update:
       clear_incr = full_hash && !incr_hash = NULL && !NULL = false
  ...
  /* Clear the peer's incremental via add with full_hash but
     no incr_hash. */
  FD_TEST( fd_sspeer_selector_add( selector, key_A, addr_A, FD_SSPEER_LATENCY_UNKNOWN,
                                   cluster_full_slot, FD_SSPEER_SLOT_UNKNOWN,
...
test_invalid_clear_and_best_sentinel( fd_sspeer_selector_t * selector,
  ...
  /* update_on_resolve with invalid clearing args (full_hash set,
     incr_hash NULL, but incr_slot != FD_SSPEER_SLOT_UNKNOWN) should
     return FD_SSPEER_UPDATE_ERR_INVALID_ARG. */
  FD_TEST( fd_sspeer_selector_update_on_resolve( selector, key_A,
                                                 cluster_full_slot, cluster_incr_slot,
                                                 full_hash, NULL )==FD_SSPEER_UPDATE_ERR_INVALID_ARG );

  /* Peer A should be unaffected (not corrupted). */
  fd_sspeer_t best = fd_sspeer_selector_best( selector, 0, FD_SSPEER_SLOT_UNKNOWN );
  FD_TEST( best.addr.l==addr_A.l );
  FD_TEST( best.full_slot==cluster_full_slot );
  FD_TEST( best.incr_slot==cluster_incr_slot );
  ...
  /* fd_sspeer_selector_add with invalid clearing args (full_hash
     set, incr_hash NULL, incr_slot != FD_SSPEER_SLOT_UNKNOWN) should
     return FD_SSPEER_SCORE_INVALID for an existing peer. */
  FD_TEST( fd_sspeer_selector_add( selector, key_A, addr_A, FD_SSPEER_LATENCY_UNKNOWN,
                                   cluster_full_slot, cluster_incr_slot,
                                   full_hash, NULL )==FD_SSPEER_SCORE_INVALID );

  /* Peer A should still be unaffected. */
  best = fd_sspeer_selector_best( selector, 0, FD_SSPEER_SLOT_UNKNOWN );
  FD_TEST( best.addr.l==addr_A.l );
  FD_TEST( best.full_slot==cluster_full_slot );
  FD_TEST( best.incr_slot==cluster_incr_slot );
  ...
  /* fd_sspeer_selector_add with invalid clearing args for a NEW peer
     (full_hash set, incr_hash NULL, incr_slot != FD_SSPEER_SLOT_UNKNOWN)
     should also return FD_SSPEER_SCORE_INVALID and not create the peer. */
